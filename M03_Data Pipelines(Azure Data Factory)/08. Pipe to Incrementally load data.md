...
Incremental load means loading only the new or changed data from a source system into a target system instead of loading the entire data every time.
...
6. Select **LastLoadDate** value as **@activity('Get_max_transaction_date_from_source').output.firstRow.max_hiredate**
7. Select **TableName** value as **@activity('Get_last_load_date').output.firstRow.TableName**
