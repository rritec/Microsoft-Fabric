...
## Home work
can you think and redo 06. pipe_Load_file_if_available.md exercise using switch(instead of if) to handle file exists check and also filetype check 
hint: think to write below expression inside switch
```json
@if(
    activity('Get Metadata1_file_exists').output.exists,
    if(
        or(
            equals(toLower(last(split(item().FileName, '.'))), 'xlsx'),
            equals(toLower(last(split(item().FileName, '.'))), 'xls')
        ),
        'excel',
        toLower(last(split(item().FileName, '.')))
    ),
    'skip'
)
```
